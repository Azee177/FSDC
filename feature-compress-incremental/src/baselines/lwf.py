...
def parse_args() -> argparse.Namespace:  # 定义命令行解析函数
    parser = argparse.ArgumentParser(description='增量基线：LwF (Learning without Forgetting)')  # 创建解析器
    ...
    parser.add_argument('--temperature', type=float, default=2.0, help='指定蒸馏温度T')  # 添加温度参数
    parser.add_argument('--alpha', type=float, default=0.7, help='指定蒸馏损失权重')  # 添加损失权重
...
def load_configs(args: argparse.Namespace) -> Dict:  # 定义配置合并函数
    ...
    merged['alpha'] = args.alpha  # 写入损失权重
...
def main() -> None:  # 定义主函数
    ...
    log_path = setup_logging(args.save_dir, 'lwf.log')  # 配置日志文件
    ...
    old_class_names = old_class_names[:old_class_count]  # 截断多余名称
    ...
    if args.use_cache:  # 若使用缓存
        new_cache = encoder.extract_and_cache(train_loader_new, 'train_new', cfg.get('image_size', 224), return_patch, True)  # 加载或生成新类训练特征
        ...
        new_cache = encoder.extract_and_cache(train_loader_new, 'train_new_lwf', cfg.get('image_size', 224), return_patch, False)  # 生成新类训练特征
        val_base_cache = encoder.extract_and_cache(val_loader_base, 'val_lwf', cfg.get('image_size', 224), return_patch, False)  # 生成旧类验证特征
        val_new_cache = encoder.extract_and_cache(val_loader_new, 'val_new_lwf', cfg.get('image_size', 224), return_patch, False)  # 生成新类验证特征
    ...
    raw_new_ids = {int(v) for v in new_cache['labels'].tolist()}  # 收集训练标签
    ...
    if len(new_class_names) < len(sorted_new_ids):  # 若新类名称不足
        new_class_names.extend([f'new_cls_{i}' for i in range(len(new_class_names), len(sorted_new_ids))])  # 补全新类名称
    elif len(new_class_names) > len(sorted_new_ids):  # 若新类名称多余
        new_class_names = new_class_names[:len(sorted_new_ids)]  # 截断
    total_classes = old_class_count + len(new_class_names)  # 计算总类别数
    class_names = old_class_names + new_class_names  # 合并类别名称
    id_to_idx = {cid: old_class_count + idx for idx, cid in enumerate(sorted_new_ids)}  # 构建标签映射
    train_labels = torch.tensor([id_to_idx[int(cid)] for cid in new_cache['labels'].tolist()], dtype=torch.long)  # 映射训练标签
    val_new_labels = torch.tensor([id_to_idx[int(cid)] for cid in val_new_cache['labels'].tolist()], dtype=torch.long)  # 映射验证标签
    train_dataset = TensorDataset(new_cache['features'].float(), train_labels.long())  # 构建训练数据集
    ...
    for epoch in range(1, cfg['epochs'] + 1):  # 遍历训练轮次
        ...
        logger.info(f'[LwF] Epoch {epoch}: top1={acc[1]:.4f} top5={acc[5]:.4f}')  # 打印日志
        if acc[1] > best_top1:  # 判断是否刷新
            best_top1 = acc[1]  # 更新最佳Top1
            torch.save({'classifier': classifier.state_dict(), 'config': cfg}, args.save_dir / 'best_lwf.ckpt')  # 保存最佳权重

    torch.save({'classifier': classifier.state_dict(), 'config': cfg}, args.save_dir / 'last_lwf.ckpt')  # 保存最终权重

    confusion = build_confusion(logits_tensor, labels_tensor, total_classes)  # 构建混淆矩阵
    plot_confusion(confusion, class_names, args.save_dir / 'confusion_lwf.png')  # 绘制混淆矩阵
    save_json(confusion_to_dict(confusion, class_names), args.save_dir / 'confusion_lwf.json')  # 保存混淆矩阵
    ...
    summary_metrics['alpha'] = alpha  # 写入权重
    save_json(summary_metrics, args.save_dir / 'metrics_lwf.json')  # 保存指标

...
from ..utils.common import ensure_dir, load_yaml_config, set_seed, get_device, save_json, log_experiment_info, count_parameters, setup_logging  # 导入通用工具
...
def main() -> None:  # 定义主函数
    ...
    log_path = setup_logging(args.save_dir, 'linear_probe.log')  # 配置日志文件
    ...
    max_train = int(train_cache['labels'].max().item()) if train_cache['labels'].numel() > 0 else -1  # 训练集最大标签
    max_val = int(val_cache['labels'].max().item()) if val_cache['labels'].numel() > 0 else -1  # 验证集最大标签
    num_classes = max(max_train, max_val) + 1  # 根据实际标签确定类别数
    class_names = list(cfg.get('class_names', []))  # 获取类别名称列表
    if len(class_names) < num_classes:  # 若名称不足
        class_names.extend([f'cls_{i}' for i in range(len(class_names), num_classes)])  # 补齐占位名称
    else:
        class_names = class_names[:num_classes]  # 截断多余名称
    cfg['class_names'] = class_names  # 更新配置中的类别名称
    ...
    for epoch in range(1, cfg['epochs'] + 1):  # 遍历训练轮次
        ...
        logger.info(f'Epoch {epoch}: val_loss={val_loss:.4f} top1={acc[1]:.4f} top5={acc[5]:.4f}')  # 打印日志

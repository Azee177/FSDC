...
def parse_args() -> argparse.Namespace:  # 定义命令行解析函数
    parser = argparse.ArgumentParser(description='增量基线：微调 + 回放')  # 创建解析器
    ...
    parser.add_argument('--resume_ckpt', type=Path, required=True, help='指定线性探针权重路径')  # 添加权重参数
    ...
    parser.add_argument('--replay_per_class', type=int, default=5, help='指定每类回放样本数')  # 添加回放数量
...
def sample_replay(features: torch.Tensor, labels: torch.Tensor, per_class: int, num_classes: int) -> torch.Tensor:  # 定义回放索引采样函数
    collected = []  # 初始化索引列表
    for cid in range(num_classes):  # 遍历旧类别
        mask = (labels == cid).nonzero(as_tuple=False).view(-1)  # 找到对应索引
        if mask.numel() == 0:  # 判断是否存在样本
            continue  # 无样本则跳过
        choice = mask[torch.randperm(mask.numel())[:per_class]]  # 随机采样
        collected.append(choice)  # 追加索引
    if not collected:  # 判断是否采样到数据
        return torch.empty(0, dtype=torch.long)  # 返回空张量
    return torch.cat(collected, dim=0)  # 拼接索引
...
def main() -> None:  # 定义主函数
    ...
    sorted_items = sorted(class_map.items(), key=lambda kv: kv[1])  # 按映射索引排序
    ...
    if 'class_names' in cfg:
        old_class_names = list(cfg['class_names'])  # 使用任务配置中的旧类别名称
        old_class_count = len(old_class_names)  # 统计旧类别数量
    else:
        old_class_count = max(0, len(class_map) - len(new_class_names))  # 若未提供则根据映射推断
    ...
    replay_idx = sample_replay(base_cache['features'], base_cache['labels'], cfg['replay_per_class'], old_class_count)  # 采样回放索引
    replay_feats = base_cache['features'][replay_idx] if replay_idx.numel() > 0 else torch.empty(0)  # 获取回放特征
    replay_labels = base_cache['labels'][replay_idx] if replay_idx.numel() > 0 else torch.empty(0, dtype=torch.long)  # 获取回放标签
    new_labels = torch.full((new_cache['features'].shape[0],), old_class_count, dtype=torch.long)  # 构建新类标签
    if replay_feats.numel() > 0:  # 判断是否存在回放
        train_features = torch.cat([new_cache['features'], replay_feats], dim=0)  # 合并特征
        train_labels = torch.cat([new_labels, replay_labels], dim=0)  # 合并标签
    else:  # 否则仅使用新类
        train_features = new_cache['features']  # 使用新类特征
        train_labels = new_labels  # 使用新类标签
    train_dataset = TensorDataset(train_features.float(), train_labels.long())  # 构建训练特征数据集
    ...
    val_dataset = TensorDataset(val_features.float(), val_labels.long())  # 构建验证特征数据集
    val_loader = DataLoader(val_dataset, batch_size=cfg['batch_size'], shuffle=False)  # 创建验证加载器
    checkpoint = torch.load(args.resume_ckpt, map_location='cpu')  # 加载线性探针权重
    classifier = LinearClassifier(encoder.output_dim(False), old_class_count)  # 创建线性分类器
    classifier.load_state_dict(checkpoint['classifier'])  # 恢复旧参数
    classifier.expand_classes(len(new_class_names))  # 扩展输出维度
    ...
    torch.save({'classifier': classifier.state_dict(), 'config': cfg}, args.save_dir / 'last_finetune.ckpt')  # 保存最终权重
